...
let HeaderComponent = {
    ...
    async init(containerId) {
        ...
        await this.initLang(container);
        this.renderNav(container);
        this.renderOverlayPlatforms(container);
    ...
    async loadHTML() {
        try {
            ...
            console.error(e);
            return `<nav class="nav"><div class="nav-inner"><span class="nav-name">Rezo Mapou</span></div></nav>`;
        }
    },

    async initLang(container) {
        // Load lang CSS + JS then init LangComponent into #lang-container
    ...
    renderNav(container) {
        ...
        if (nav) {
            nav.innerHTML = '';
            navItems.forEach(item => nav.appendChild(makeLink(item)));
        }
        if (overlay) {
            overlay.innerHTML = '';
            navItems.forEach(item => overlay.appendChild(makeLink(item)));
        }
    },

    renderOverlayPlatforms(container) {
        const slot = container.querySelector('#nav-overlay-platforms');
        if (!slot) return;
        const platforms = (config_const.DATA?.PLATFORMS || []).filter(p => p.url);
