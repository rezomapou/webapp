...
let HeaderComponent = {
    ...
    renderNav() {
        ...
        if (!nav) return;
        nav.innerHTML = (config_const.DATA?.NAV_ITEMS || []).map(item =>
    ...
    renderOverlayPlatforms() {
        const el = document.getElementById('nav-overlay-platforms');
        if (!el) return;
    ...
    async initLang() {
        ...
        await LoaderEngine.loadCSS('/components/' + langCfg.css);
        await LoaderEngine.loadScript('/components/' + langCfg.js);
